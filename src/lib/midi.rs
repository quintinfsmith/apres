...
impl MIDITrack {
    ...
    pub fn get_active_tick(&self, n: usize) -> usize {
    ...
    pub fn get_event_count(&self, tick: usize) -> usize {
    ...
    pub fn get_pair_map(&self) -> Vec<(usize, u64)> {
...
impl MIDI {
    ...
    pub fn get_tick_length(&self, track: usize, tick: usize) -> usize {
        let length: usize;
    ...
    pub fn get_nth_event_id_in_tick(&self, track: usize, tick: usize, n: usize) -> Result<u64, u32> {
        match self.tracks.get(track) {
            Some(miditrack) => {
    ...
    pub fn get_track(&self, track_number: usize) -> Option<&MIDITrack> {
    ...
    pub fn get_track_mut(&mut self, track_number: usize) -> Option<&MIDITrack> {
                ...
                match a {
                    0x02 => {
                        match std::str::from_utf8(dump.as_slice()) {
                            Ok(textdump) => {
                                mlo.insert_event(track, *current_deltatime, CopyRightNoticeEvent::new(textdump.to_string()));
                            }
                            Err(e) => {}
                        };
                    }
                    0x03 => {
                        match std::str::from_utf8(dump.as_slice()) {
                            Ok(textdump) => {
                                mlo.insert_event(track, *current_deltatime, TrackNameEvent::new(textdump.to_string()));
                            }
                            Err(e) => {}
                        };
                    }
                    0x04 => {
                        match std::str::from_utf8(dump.as_slice()) {
                            Ok(textdump) => {
                                mlo.insert_event(track, *current_deltatime, InstrumentNameEvent::new(textdump.to_string()));
                            }
                            Err(e) => {}
                        };
                    }
                    0x05 => {
                        match std::str::from_utf8(dump.as_slice()) {
                            Ok(textdump) => {
                                mlo.insert_event(track, *current_deltatime, LyricEvent::new(textdump.to_string()));
                            }
                            Err(e) => {}
                        };
                    }
                    0x06 => {
                        match std::str::from_utf8(dump.as_slice()) {
                            Ok(textdump) => {
                                mlo.insert_event(track, *current_deltatime, MarkerEvent::new(textdump.to_string()));
                            }
                            Err(e) => {}
                        };
                    }
                    0x07 => {
                        match std::str::from_utf8(dump.as_slice()) {
                            Ok(textdump) => {
                                mlo.insert_event(track, *current_deltatime, CuePointEvent::new(textdump.to_string()));
                            }
                            Err(e) => {}
                        };
                    }
                    ...
                    0x59 => {
                        mlo.insert_event(track, *current_deltatime, KeySignatureEvent::from_mi_sf(dump[0], dump[1]));
                    }
                    0x7F => {
                        mlo.insert_event(track, *current_deltatime, SequencerSpecificEvent::new(dump));
                    }
                    _ => {
                    }
                }
